...
function paletteInjection() {
  ...
  return { name: 'amber-palette-injection', transformIndexHtml: { order: 'post', handler(html) { return html.replace(/(<meta name="theme-color" content=")[^"]+(")/, `$1${surface}$2`).replace('</head>', `    <style id="bw-palette">${css}</style>\n  </head>`) } } }
}
