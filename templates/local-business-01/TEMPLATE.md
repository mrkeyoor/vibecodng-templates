...
Amber Room is a complete warm-minimal landing template for a fictional neighborhood cafe. It follows the `landing-01` assembly pattern: every section adapts a verified entry from the `mrkeyoor.components` index, each section file carries its attribution, the indexed result records live under `provenance/results/`, and the palette is controlled by the four `--bw-*` variables.
...
Source structures were retained while copy, spacing, imagery, and interaction were adapted for Amber Room. Remote gallery images use fixed Unsplash URLs with meaningful alt text. Reveals run once with short staggered delays. Hover motion is limited to a few pixels, and the reduced-motion query disables animation, smooth scrolling, reveal transforms, and hover transforms while keeping all content visible.
